...
**Why I'm keeping this in the README:** a real-world least-privilege gap that's easy to hit any time a role's initial permission set was written to satisfy schema creation but not full crawler behavior. It's also a good example of reading a CloudWatch log carefully before assuming a failure means bad data - three of the four log lines were successes, and only the last line was the actual problem.
