...
- Since AWS split new accounts into Free/Paid Plan (July 2025), <strong>Redshift is excluded from the Free Plan outright</strong> - not rate-limited, not a trial restriction, just unavailable until the account is upgraded to Paid.
- Glue's `AccessDeniedException` is a <strong>backend fraud-prevention hold</strong> AWS places on new/recently-upgraded accounts for certain compute-provisioning APIs (crawler/job creation specifically), to block abuse like crypto-mining. It happens before IAM is ever evaluated, so no policy change fixes it - the exact same Terraform that failed one day applied cleanly with zero code changes once AWS lifted the hold.
